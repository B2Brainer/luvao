...
const EXTRA_CATEGORY_BY_PRODUCT = {
    atun: 'Proteínas',
    manzana: 'Frutas',
    pera: 'Frutas',
    papaya: 'Frutas',
...
let ComparisonService = class ComparisonService {
    scrapedClient;
    productClient;
    constructor(scrapedClient, productClient) {
    ...
    async compareByProduct(product) {
        ...
        const canonicalRaw = raw.map((p) => this.toCanonicalProduct(p, product));
    ...
    async optimizeShoppingList(items, options = {}) {
        const isManualList = Boolean(items && items.length > 0);
        const hasScenarioOptions = Boolean((options.periodDays && options.periodDays > 0) ||
            (options.targetCalories && options.targetCalories > 0));
        const requestedItems = isManualList ? items : await this.getDefaultResearchBasket();
        const allRaw = await this.scrapedClient.searchByFilters({ availability: true });
        const canonicalAll = allRaw.map((p) => this.toCanonicalProduct(p));
        if (!isManualList) {
            return this.optimizeBasketByCalories(requestedItems, canonicalAll, options);
        }
        if (hasScenarioOptions) {
            return await this.optimizeCustomListByCalories(requestedItems, canonicalAll, options);
        }
        ...
        const total = selectedLines.reduce((acc, l) => acc + l.subtotal, 0);
        const byStore = {};
        for (const line of selectedLines) {
            ...
            byStore[store] = (byStore[store] ?? 0) + line.subtotal;
        }
    ...
    async optimizeCustomListByCalories(requestedItems, canonicalAll, options) {
        ...
        const drafts = requestedItems.map((item) => this.buildDraftLine(item.product, item.quantity && item.quantity > 0 ? item.quantity : 1, this.inferCategoryForProduct(item.product, categoryLookup), canonicalAll));
    ...
    optimizeBasketByCalories(requestedItems, canonicalAll, options) {
        const periodDays = options.periodDays && options.periodDays > 0
            ? Math.round(options.periodDays)
            : DEFAULT_PERIOD_DAYS;
        const targetCalories = options.targetCalories && options.targetCalories > 0
            ? Math.round(options.targetCalories)
            : Math.round(DEFAULT_DAILY_CALORIES * periodDays);
        const drafts = requestedItems.map((item) => this.buildDraftLine(item.product, item.quantity && item.quantity > 0 ? item.quantity : 1, item.category ?? UNCATEGORIZED_LABEL, canonicalAll));
        const primaryDrafts = options.restrictedStore
            ? drafts.map((line) => this.cloneDraftLine(line, options.restrictedStore))
            : drafts;
    ...
    filterCandidatesByRestrictedStore(candidates, restrictedStore) {
        if (!restrictedStore) {
            return candidates;
        }
        return candidates.filter((candidate) => this.storeKey(candidate.storeName) === this.storeKey(restrictedStore));
    }
    buildDraftLine(product, requestedQuantity, category, canonicalAll) {
    ...
    serializeOptimizeLine(line) {
        const { candidates, requestedQuantity, ...rest } = line;
        return rest;
    }
    resolveCalorieScenario(drafts, targetCalories, options) {
        ...
        for (const [category, categoryLines] of categoryGroups.entries()) {
            ...
            const caloricLines = categoryLines.filter((line) => line.candidates.some((candidate) => candidate.nutrition.calories !== null && candidate.nutrition.calories > 0));
        ...
        const totalEstimated = selectedLines.reduce((acc, line) => acc + line.subtotal, 0);
        const plannedCalories = selectedLines.reduce((acc, line) => acc + (line.plannedCalories ?? 0), 0);
        const estimatedByStore = {};
        const plannedByCategory = {};
        for (const line of selectedLines) {
            const store = line.selected?.storeName;
            if (store) {
                estimatedByStore[store] = (estimatedByStore[store] ?? 0) + line.subtotal;
            }
    ...
    buildCategoryTargets(targetCalories, drafts, plannedByCategory) {
    ...
    buildStoreScenarioSummaries(drafts, targetCalories, options) {
    ...
    async getResearchCategoryLookup() {
    ...
    inferCategoryForProduct(product, categoryLookup) {
    ...
    inferCategoryFromRules(normalized, candidateTokens) {
    ...
    getCandidateRanking(product, canonicalAll) {
        const targetTokens = this.getTargetMatchTokens(product);
        const relevant = canonicalAll
            .map((p) => {
    ...
    pickBestCalorieValue(candidates) {
        const pricedCalories = candidates.filter((candidate) => candidate.price !== null &&
            candidate.price > 0 &&
    ...
    pickLowestPriceCandidate(candidates) {
        const pricedCandidates = candidates.filter((candidate) => candidate.price !== null && candidate.price > 0);
        if (pricedCandidates.length === 0) {
            return null;
        }
        const comparableByPresentation = pricedCandidates.filter((candidate) => candidate.pricePerUnit !== null &&
        ...
        return [...candidatesToSort].sort((a, b) => {
            if (comparableByPresentation.length > 0) {
                const byUnitPrice = a.pricePerUnit - b.pricePerUnit;
                if (byUnitPrice !== 0) {
                    return byUnitPrice;
                }
            }
            const byRawPrice = a.price - b.price;
            if (byRawPrice !== 0) {
                return byRawPrice;
            }
    ...
    withNutritionTarget(product, nutritionTarget) {
    ...
    pickCaloriePlanForTarget(candidates, targetCalories) {
        const plans = candidates
            ...
            const subtotal = candidate.price * quantity;
    ...
    toCanonicalProduct(raw, nutritionTarget = raw.query) {
    ...
    normalizeTextForPresentation(value) {
        return value
            .toLowerCase()
            .normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '')
    ...
    toBasePresentationAmount(amount, unit) {
    ...
    estimateNutrition(productQuery, presentation, normalizedName) {
        ...
        const baseUnit = presentation.baseUnit;
    ...
    resolveCalorieReferenceKey(productQuery) {
    ...
    caloriesPerBaseUnit(reference, baseUnit, productKey, normalizedName) {
    ...
    computeTextMatchScore(targetTokens, candidateTokens) {
        ...
        if (isEggQuery) {
            ...
            const hasEggNonFoodSignal = candidateTokens.some((token) => EGG_NON_FOOD_STEMS.some((stem) => token.includes(stem)));
    ...
    hasBaseDerivativeNoise(targetNorm, tokens, normalizedName) {
    ...
    async getDefaultResearchBasket() {
        ...
        return (await this.productClient.getProductNames()).map((name) => ({
            product: name,
            quantity: 1,
        }));
    }
...
exports.ComparisonService = ComparisonService = __decorate([
    ...
    __metadata("design:paramtypes", [scraped_client_1.ScrapedClient,
